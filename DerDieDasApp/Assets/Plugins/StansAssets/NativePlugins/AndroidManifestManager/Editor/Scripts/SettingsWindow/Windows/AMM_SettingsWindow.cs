...
namespace SA.Android.Manifest
{
    ...
    {
        static AMM_BaseTemplate m_parentTemplate = null;
        ...
        public static void AddValueDialog(AMM_BaseTemplate parent)
        {
            ...
            var dialog = CreateInstance<AMM_AddValueDialog>();
        ...
        static void OnAddValueClick(string key, string value)
        {
        ...
        static void OnValueDlgClose(object dialog)
        {
            m_parentTemplate = null;

            var dlg = dialog as AMM_AddValueDialog;
        ...
        public static void AddPropertyDialog(AMM_BaseTemplate parent)
        {
            ...
            var dialog = CreateInstance<AMM_AddPropertyDialog>();
        ...
        static void OnAddPropertyClick(string tag)
        {
            var property = new AMM_PropertyTemplate(tag);
            m_parentTemplate.AddProperty(tag, property);
        }

        static void OnPropertyDlgClose(object dialog)
        {
            m_parentTemplate = null;

            var dlg = dialog as AMM_AddPropertyDialog;
        ...
        public static void DrawPropertiesInternal(AMM_BaseTemplate parent)
        {
            ...
            foreach (var property in parent.Properties[key])
            {
                if (parent is AMM_ActivityTemplate)
                {
                    var activity = parent as AMM_ActivityTemplate;
                    if (activity.IsLauncherProperty(property)) continue;
                }
                ...
                if (property.IsOpen)
                {
                    ...
                    foreach (var k in property.Values.Keys)
                    {
                        EditorGUILayout.Space();
                        EditorGUILayout.BeginHorizontal();
                    ...
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.Space();
                    if (GUILayout.Button("Add Value", GUILayout.Width(100.0f))) AddValueDialog(property);
                    if (GUILayout.Button("Add Property", GUILayout.Width(100.0f))) AddPropertyDialog(property);
                    EditorGUILayout.Space();
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.Space();
                }
