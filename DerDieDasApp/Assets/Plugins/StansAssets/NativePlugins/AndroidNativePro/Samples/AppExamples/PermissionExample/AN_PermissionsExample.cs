...
#pragma warning disable 649
    [SerializeField]
    Button m_checkSelfPermission;
    [SerializeField]
    Button m_shouldShowRequestPermissionRationale;
    [SerializeField]
    Button m_requestPermissions;
#pragma warning restore 649
    ...
    public void Awake()
    {
        ...
        {
            var result = AN_PermissionsManager.ShouldShowRequestPermissionRationale(AMM_ManifestPermission.READ_EXTERNAL_STORAGE);
            Debug.Log("ShouldShowRequestPermissionRationale: " + result.ToString());
        });

        m_requestPermissions.onClick.AddListener(() =>
        {
            var permissions = new List<AMM_ManifestPermission>();
