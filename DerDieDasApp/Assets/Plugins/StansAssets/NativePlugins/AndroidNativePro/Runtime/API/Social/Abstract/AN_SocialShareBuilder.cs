...
namespace SA.Android.Social
{
    public abstract class AN_SocialShareBuilder
    {
        ...
        protected List<string> m_links = new List<string>();
        AN_Intent m_shareIntent;
        string m_packageName = string.Empty;

        protected abstract AN_Intent MakeSharingIntent();
        AN_ProxyActivity m_proxyActivity;
        ...
        void ShowSharingDialog(Action callback = null)
        {
            GenerateShareIntent();
        ...
        {
            if (!string.IsNullOrEmpty(m_text)) AppendText();

            if (m_images.Count > 0) AppendImages();

            if (m_links.Count > 0) AppendURL();
        ...
        {
            if (filters.Length == 0) return AN_Intent.CreateChooser(ShareIntent, title);
            ...
            if (intentShareList.Count == 0)
            {
                ...
                var filterList = "";
                foreach (var f in filters) filterList += f + ";";
            ...
            else
            {
                ...
                if (intentShareList.Count == 1) return intentShareList[0];
        ...
        void AppendURL()
        {
            ...
            {
                ShareIntent.SetAction(AN_Intent.ACTION_SEND_MULTIPLE);
                ShareIntent.PutExtra(AN_Intent.EXTRA_STREAM, m_links.ToArray());
            }
            else
            {
                ShareIntent.SetAction(AN_Intent.ACTION_SEND);
                ShareIntent.PutExtra(AN_Intent.EXTRA_STREAM, m_links[0]);
            }
        ...
        protected AN_Intent ShareIntent => m_shareIntent ?? (m_shareIntent = new AN_Intent());
    }
}
