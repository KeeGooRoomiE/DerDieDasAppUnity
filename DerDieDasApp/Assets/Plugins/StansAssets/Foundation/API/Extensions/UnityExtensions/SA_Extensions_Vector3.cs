...
public static class SA_Extensions_Vector3
{
    ...
    public static Vector3 Average(this Vector3[] f)
    {
        ...
        for (var i = 0; i < f.Length - 1; i++) // If a value is 0.0f it means we didn't collect that sample yet, therefore, we'll ignore it in our average
            if (f[i] != Vector3.zero)
            {
                temp += f[i];
                validValues++;
