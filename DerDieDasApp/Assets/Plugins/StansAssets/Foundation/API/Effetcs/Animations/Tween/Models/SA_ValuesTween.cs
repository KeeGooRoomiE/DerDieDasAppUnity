...
namespace SA.Foundation.Animation
{
    ...
    {
        readonly SA_Event m_onComplete = new SA_Event();
        readonly SA_Event<float> m_onValueChanged = new SA_Event<float>();
        readonly SA_Event<Vector3> m_onVectorValueChanged = new SA_Event<Vector3>();

        public bool DestoryGameObjectOnComplete = true;

        float FinalFloatValue;
        Vector3 FinalVectorValue;
        ...
        {
            if (gameObject == null || gameObject.Equals(null)) return;
