...
namespace SA.Foundation.Async
{
    public static class SA_AsyncLoader
    {
        ...
        static IEnumerator ResourceLoadCoroutine<T>(string path, Action<T> callback) where T : UnityEngine.Object
        {
            ...
            {
                var resource = UnityEngine.Object.Instantiate(request.asset) as T;
                callback(resource);
            }
        }
        ...
        static IEnumerator ResourceLoadTextureCoroutine<T>(string path, Action<T> callback) where T : UnityEngine.Object
        {
