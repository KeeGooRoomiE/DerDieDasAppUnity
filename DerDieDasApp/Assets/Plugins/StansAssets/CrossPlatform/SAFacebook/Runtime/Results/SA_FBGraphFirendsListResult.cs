...
namespace SA.Facebook
{
    public class SA_FBGraphFirendsListResult : SA_FB_Result
    {
        ...
        public SA_FBGraphFirendsListResult(IGraphResult graphResult)
            : base(graphResult)
        {
            if (m_error == null)
                try
                {
                    ...
                    var f = JSON[FriendsListKey] as IDictionary;
                    var flist = f["data"] as IList;
        ...
        public virtual string FriendsListKey => "friends"; // invitable_friends for the invitable firends request
    }
}
