...
namespace SA.CrossPlatform.Editor
{
    class UM_GameServicesUI : UM_ServiceSettingsUI
    {
        ...
        static readonly GUIContent s_AndoirdSavedGamesContent = new GUIContent("Saved Games API [?]:", "Without Games API enabled only basic sing in flow can be used");
        static readonly GUIContent s_AndoirdRequestProfileContent = new GUIContent("Request Profile [?]:", "Request User Profile will be added to sing-in builder");
        static readonly GUIContent s_AndoirdRequestEmailContent = new GUIContent("Request Email [?]:", "Request User Email will be added to sing-in builder");
        static readonly GUIContent s_AndoirdRequestServerAuthCodeContent = new GUIContent("Request Server Auth Code[?]:", "Request  Server Auth Code will be added to sing-in builder");
        ...
        void Settings()
        {
            using (new SA_WindowBlockWithSpace(new GUIContent("Settings")))
            {
                using (new SA_H2WindowBlockWithSpace(new GUIContent("ANDROID")))
                {
                    ...
                    if (UM_Settings.Instance.AndroidRequestServerAuthCode) UM_Settings.Instance.AndroidGMSServerId = SA_EditorGUILayout.TextField("Server Id", UM_Settings.Instance.AndroidGMSServerId);
                }
