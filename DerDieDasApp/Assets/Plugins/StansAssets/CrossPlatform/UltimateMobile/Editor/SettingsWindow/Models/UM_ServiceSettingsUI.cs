...
namespace SA.CrossPlatform.Editor
{
    abstract class UM_ServiceSettingsUI : SA_ServiceLayout
    {
        ...
        public override SA_iAPIResolver Resolver => null;
        ...
        protected virtual void DrawDefaultBlocks()
        {
            if (m_platforms.Count > 0)
            {
                using (new SA_WindowBlockWithSpace(new GUIContent("Plugins"), 5))
                {
                    ...
                    {
                        var clicked = platfrom.Layout.OnGUI();
                        if (clicked) OpenPlatfromUI(platfrom);
                    }
                }
            }
            else
        ...
        void OpenPlatfromUI(UM_ServicePlatformInfo platformInfo)
        {
        ...
        {
            foreach (var platfrom in m_platforms) platfrom.Layout.SetActiveState(platfrom.Settings.IsEnabled);
        }
    }
}
